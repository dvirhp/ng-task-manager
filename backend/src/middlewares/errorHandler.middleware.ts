...
export const errorHandler = (
  ...
  if (err?.code === 11000) {
    ...
    return res
      .status(409)
      .json(errorResponse(`Duplicate value for ${field}`, 409));
  }
  ...
  if (err instanceof mongoose.Error.ValidationError) {
    ...
    return res
      .status(400)
      .json(
        errorResponse("Mongoose validation failed", 400, { errors: messages }),
      );
  }
  ...
  if (err instanceof mongoose.Error.CastError) {
    return res
      .status(400)
      .json(errorResponse(`Invalid ${err.path}: ${err.value}`, 400));
  }
  ...
  return res
    .status(500)
    .json(errorResponse("Something went wrong. Please try again later.", 500));
};
